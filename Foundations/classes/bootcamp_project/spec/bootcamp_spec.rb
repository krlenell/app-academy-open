...
describe 'Bootcamp' do
  ...
  describe 'PART 1' do
    describe '#initialize' do
      it 'accepts a name (string), slogan (string), and student_capacity (number) as args' do
        bootcamp
      end

      it 'sets the instance variables @name, @slogan, and @student_capacity' do
      ...
      it 'sets the instance variables @teachers and @students to empty arrays' do
        expect(bootcamp.instance_variable_get(:@teachers)).to eq([])
        expect(bootcamp.instance_variable_get(:@students)).to eq([])
      end

      it 'sets the instance variable @grades to an empty hash where the default values are distinct empty arrays' do
    ...
    describe '#enroll' do
      ...
      context 'when the number of enrolled students is at @student_capacity' do
        it 'does not add the student to @students' do
          8.times { bootcamp.enroll('Alice') }
          expect(bootcamp.students.length).to eq(6)
        end

        it 'returns false since the enrollment was not successful' do
          expect(bootcamp.enroll('student one')).to eq(true)
          expect(bootcamp.enroll('student two')).to eq(true)
    ...
    describe '#enrolled?' do
      it 'accepts a student (string) and return a boolean indicating whether the student is enrolled in the bootcamp' do
        bootcamp.enroll('Alice')
  ...
  describe 'PART 2' do
    describe '#student_to_teacher_ratio' do
      it 'returns an integer representing the ratio between # students to 1 teacher rounded down to the nearest integer' do
        bootcamp.hire('Jeff')
        bootcamp.hire('Matthias')
    ...
    describe '#add_grade' do
      it "accepts a student (string) and a grade (number) and add that grade to the student's grades array inside of the @grades hash" do
        bootcamp.add_grade('Alice', 100)
      end

      context 'when the student is already enrolled' do
        it "adds the grade to the student's grades array inside of the @grades hash" do
          bootcamp.enroll('Alice')
          bootcamp.enroll('Bob')
          bootcamp.add_grade('Alice', 100)
    ...
    describe '#num_grades' do
      it 'accepts a student (string) and return the number of grades they have' do
        bootcamp.enroll('Alice')
        bootcamp.add_grade('Alice', 100)
        bootcamp.add_grade('Alice', 75)
    ...
    describe '#average_grade' do
      it 'accepts a student (string) and return a number representing their average grade rounded down to the nearest integer' do
        bootcamp.enroll('Alice')
        bootcamp.add_grade('Alice', 100)
        bootcamp.add_grade('Alice', 75)
      ...
      it 'returns nil if the student has no grades or is not enrolled' do
        bootcamp.enroll('Bob')
