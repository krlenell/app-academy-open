...
describe 'PART 2' do
  describe 'all_words_capitalized?' do
    it 'accepts an array of words as an arg' do
      expect { all_words_capitalized?(%w[Hello World]) }.not_to raise_error
    end

    context 'when all words are properly capitalized (first letter of word uppercase, other letters lowercase)' do
      it 'returns true' do
        expect(all_words_capitalized?(%w[Hello World])).to eq(true)
        expect(all_words_capitalized?(%w[Follow The Yellow Brick Road])).to eq(true)
      end
    end

    context 'when at least one word is not capitalized' do
      it 'returns false' do
        expect(all_words_capitalized?(%w[hello world])).to eq(false)
        expect(all_words_capitalized?(%w[Hello WORLD])).to eq(false)
        expect(all_words_capitalized?(%w[Follow the Yellow Brick Road])).to eq(false)
      end
    end

    it 'uses Array#all?' do
      array = %w[Hello World]
  ...
  describe 'no_valid_url?' do
    it 'accepts an array of urls as an arg' do
      urls = ['appacademy.io', 'sennacy.com', 'heyprogrammers.org']
      expect { no_valid_url?(urls) }.not_to raise_error
    end
  ...
  describe 'any_passing_students?' do
    it 'accepts an array of student hashes as an arg' do
      students = [
      ...
      expect { any_passing_students?(students) }.not_to raise_error
    end
