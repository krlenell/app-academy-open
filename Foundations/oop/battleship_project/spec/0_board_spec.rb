...
describe 'Board' do
  ...
  describe 'PART 1' do
    describe '#initialize' do
      it 'accepts a number, n, as an arg' do
        board
      end

      it 'sets @grid to be a 2D array with n rows and n columns' do
      ...
      it 'sets all elements of @grid to :N' do
        grid = board.instance_variable_get(:@grid)
        nil_grid = Array.new(10) { Array.new(10, :N) }
        expect(grid).to eq(nil_grid)
      end

      it 'makes each subarray of @grid refer to a distinct array' do
        grid = board.instance_variable_get(:@grid)
        expect(grid[0]).not_to be(grid[1])
      end

      it 'sets @size to be a number representing the total size of the grid (n * n)' do
    ...
    describe '#size' do
      it 'gets (return) @size' do
        expect(board.size).to be(board.instance_variable_get(:@size))
      end
    end

    describe '#[]' do
      it 'accepts an array containing a pair of indices in the form [row, column] representing a position on the @board' do
        pos = [2, 4]
        board[pos]
      end

      it 'returns the element of @grid at the given position' do
        grid = board.instance_variable_get(:@grid)
        grid[2][4] = :S
    ...
    describe '#[]=' do
      it 'accepts a position and value as args' do
        pos = [2, 4]
        # Hint:
      ...
      it 'sets the given position of @grid to the given value' do
        pos_1 = [2, 4]
        board[pos_1] = :S
        expect(board[pos_1]).to eq(:S)
    ...
    describe '#num_ships' do
      it 'returns a number representing the count of :S values in @grid' do
  ...
  describe 'PART 2' do
    describe '#attack' do
      it 'accepts a position as an arg' do
        board.attack([5, 1])
      end

      context 'when the given position of @grid has a value of :S' do
        it 'sets that value to :H' do
          pos = [2, 4]
          board[pos] = :S
        ...
        it "prints 'you sunk my battleship!'" do
          pos = [2, 4]
          board[pos] = :S
          expect { board.attack(pos) }.to output(/sunk/).to_stdout
        end

        it 'returns true to indicate the attack hit a ship' do
          pos = [2, 4]
          board[pos] = :S
      ...
      context 'when the given position of @grid does not have a value of :S' do
        it 'sets that value to :X' do
          pos = [2, 4]
          board.attack(pos)
          expect(board[pos]).to eq(:X)
        end

        it 'returns false to indicate the attack missed' do
          pos = [2, 4]
          expect(board.attack(pos)).to eq(false)
        end
      end

      it 'calls Board#[] and Board#[]= to check and set @grid' do
    ...
    describe '#place_random_ships' do
      it "randomlies set 25% of the @grid's elements to :S" do
      ...
      it "alwayses set 25% of the @grid's elements based on the @grid's dimensions" do
    ...
    describe '#hidden_ships_grid' do
      it 'returns a 2D array representing the grid where every :S is replaced with an :N' do
        board = Board.new(2)
    ...
    describe '::print_grid' do
      it 'accepts a 2D array representing a grid as an arg' do
        Board.print_grid([%i[S N], %i[X S]])
      end

      it 'prints each row of @grid so every element in a row is separated with a space' do
        expect { Board.print_grid([%i[S N], %i[X S]]) }.to output(/S N\nX S\n/).to_stdout
      end
    end

    describe '#cheat' do
      it 'calls Board::print_grid with @grid as an arg' do
        board.place_random_ships
        grid = board.instance_variable_get(:@grid)
    ...
    describe '#print' do
      it 'calls Board::print_grid with the #hidden_ships_grid as an arg' do
        board.place_random_ships
