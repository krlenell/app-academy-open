...
describe 'least_common_multiple' do
  it 'returns the least common multiple of 4 and 10' do
    expect(least_common_multiple(4, 10)).to eq(20)
    expect(least_common_multiple(10, 4)).to eq(20)
  end

  it 'returns the least common multiple of 7 and 21' do
...
describe 'most_frequent_bigram' do
  it 'returns the bigram that appears the most in the string' do
...
describe 'Hash' do
  describe '#inverse' do
    it 'swaps the keys and values in the hash' do
...
describe 'Array' do
  describe '#pair_sum_count' do
    it 'returns the number of pairs of elements that add to the given num' do
  ...
  describe '#bubble_sort' do
    context 'when no proc is given as an arg' do
      it 'sorts the array in increasing order by default' do
        expect([4, 12, 2, 8, 1, 14, 9, 25, 24, 81].bubble_sort).to eq([1, 2, 4, 8, 9, 12, 14, 24, 25, 81])
      end
    end

    context 'when a proc is given as an arg' do
      it 'sorts the array according to proc' do
        actual = [4, 12, 2, 8, 1, 14, 9, 25, 24, 81].bubble_sort do |a, b|
          a.to_s <=> b.to_s
        end
    ...
    it 'mutates the existing array' do
      array = [5, 4, 7]
