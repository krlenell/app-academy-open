...
describe 'pow(base, exponent)' do
  it 'correctlies calculate any base raised to the 0-th power' do
    expect(pow(2, 0)).to eq(1)
    expect(pow(7, 0)).to eq(1)
    expect(pow(10, 0)).to eq(1)
  end

  it 'calculates the base raised to the exponent power' do
...
describe 'lucas_number(n)' do
  it 'returns the n-th number of the Lucas Sequence' do
...
describe 'sum_array(array)' do
  it 'returns the total sum of all elements in the array' do
...
describe 'reverse_string(str)' do
  it 'returns the string with its characters in reverse order' do
