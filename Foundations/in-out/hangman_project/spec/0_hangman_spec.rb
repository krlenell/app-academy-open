...
describe 'Hangman' do
  ...
  describe 'PART 1' do
    describe '::random_word' do
      it 'returns a random word in the dictionary' do
        expect(Hangman::DICTIONARY).to include(Hangman.random_word)
      end

      it 'calls Array#sample' do
    ...
    describe '#initialize' do
      it 'calls Hangman::random_word' do
        allow(Hangman).to receive(:random_word).and_return('dog')
        expect(Hangman).to receive(:random_word)
        hangman
      end

      it 'sets @secret_word with the random word' do
        allow(Hangman).to receive(:random_word).and_return('dog')
        expect(hangman.instance_variable_get(:@secret_word)).to eq('dog')
      end

      it "sets @guess_word to be an array with the same length as the @secret_word containing '_' as each element" do
        allow(Hangman).to receive(:random_word).and_return('dog')
    ...
    describe '#remaining_incorrect_guesses' do
      it 'gets (return) @remaining_incorrect_guesses' do
    ...
    describe '#get_matching_indices' do
      it 'accepts a single char as an arg' do
        hangman.get_matching_indices('o')
      end

      it 'returns an array containing all indices of @secret_word where the char can be found' do
        allow(Hangman).to receive(:random_word).and_return('bootcamp')
    ...
    describe '#fill_indices' do
      it 'accepts a char and an array of indices' do
        allow(Hangman).to receive(:random_word).and_return('bootcamp')
        hangman.fill_indices('o', [1, 2])
      end

      it 'sets the given indices of @guess_word to the given char' do
        allow(Hangman).to receive(:random_word).and_return('bootcamp')
        hangman.fill_indices('o', [1, 2])
