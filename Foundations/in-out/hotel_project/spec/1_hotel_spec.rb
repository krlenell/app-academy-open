...
describe 'PART 2' do
  describe 'Hotel' do
    ...
    describe '#initialize' do
      it 'accepts a name (string) and hash where keys are room names (strings) and values are corresponding capacities (numbers)' do
        hotel
      end
      ...
      it 'sets @rooms to a new hash where keys are the room names from the hash arg and values are Room instances with the specified capacities' do
      ...
      it 'calls Room#initialize for each key of the hash arg' do
    ...
    describe '#name' do
      it 'gets (return) a version of @name where every word is capitalized' do
        expect(hotel.name).to eq("Hilbert's Grand Hotel")
      end

      it 'does not mutate the original @name' do
        expect(hotel.instance_variable_get(:@name)).to eq("hilbert's grand hotel")
      end
    end

    describe '#rooms' do
      it 'gets (return) @rooms' do
        expect(hotel.rooms).to be(hotel.instance_variable_get(:@rooms))
      end
    end

    describe '#room_exists?' do
      it 'accepts a room name (string) as an arg' do
    ...
    describe '#check_in' do
      ...
      context 'when the specified room does exist' do
        ...
        context 'when Room#add_occupant succeeds' do
          it "prints 'check in successful'" do
        ...
        context 'when Room#add_occupant fails' do
          it "prints 'sorry, room is full'" do
            hotel.check_in('Rick', 'Attic')
            hotel.check_in('Morty', 'Attic')
    ...
    describe '#list_rooms' do
      it 'prints out every room name followed by the number of available spaces in that room' do
        hotel.check_in('Harry', 'Under the Stairs')
        expect { hotel.list_rooms }.to output(/Basement.*4\nAttic.*2\nUnder the Stairs.*0\n/).to_stdout
      end

      it 'calls Room#available_space' do
